...
export const API = {
  ...
  async sendChannelCommand(premiseId, deviceId, channelId, command, value) {
    return request(
      `/premises/${premiseId}/devices/${deviceId}/channels/${channelId}/command`,
      {
        method: "POST",
        body: JSON.stringify({ command, value }),
      },
  ...
  async getTelegrams(premiseId, limit = 100, offset = 0) {
    return request(
      `/premises/${premiseId}/telegrams?limit=${limit}&offset=${offset}`,
    );
  },
  ...
  async getNextSubAddress(premiseId, main, middle) {
    return request(
      `/premises/${premiseId}/groups/next-sub?main=${main}&middle=${middle}`,
    );
  },
