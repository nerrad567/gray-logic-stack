...
- **File:** `devices.go:266`
- **Problem:** Device commands were always routed to a hardcoded bridge topic regardless of which protocol bridge owns the device.
- **Fix:** Added `deriveBridgeID(dev)` helper that uses the device's protocol to build the correct topic (`graylogic/command/{protocol}/{id}`). Now uses the actual device record fetched from the registry.
